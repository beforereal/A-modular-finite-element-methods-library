...
int main ( int argc, char *argv[] ){
  ...
  for(int step = 0; step<Nsteps; step++){
    ...
    if(step%Noutput == 0){
    	...
    	double Result_atomic = infinityNormAtomic(&advc);
	printf("Result Atomic=%f\n", Result_atomic);
	 
    	double t1= omp_get_wtime();
	
	double Result_critical = infinityNormCritical(&advc);
	printf("Result Critical=%f\n", Result_critical);
	 
    	double t2= omp_get_wtime();
	
	double Result_reduction = infinityNormReduction(&advc);
	printf("Result Reduction=%f\n", Result_reduction);
...
#pragma omp parallel for default(none) private(i, j) shared(solver, tstep, msh, stage)
for(int j=0; j<msh->NY; j++){
    ...
    for(int i=0; i<msh->NX; i++){
     ...
     // double *vector = (double *)malloc(solver->q*sizeof(double)); 
     // vector[idn]=solver->q[idn] ;
    
	// printf("The vector is: %f\n", vector[idn]);
      tstep->resq[idn] = resq; 
      tstep->rhsq[idn] = rhsq;
    } 
...
double infinityNormAtomic(solver_t *solver) {
    ...
    double *vector = (double *)malloc(msh->Nnodes * sizeof(double));

    int a = msh->Nnodes;

    #pragma omp parallel shared(solver, vector, maxVal, a) private(i)
    {
        double privateMaxVal = 0.0;

        #pragma omp for
        for (i = 0; i < a; i++) {
            vector[i] = solver->q[i];
            double val = fabs(vector[i]);
            privateMaxVal = fmax(privateMaxVal, val);
        }
	if (privateMaxVal > maxVal) {
                
        #pragma omp atomic write
            maxVal = privateMaxVal;
            			    }
